...
@dag(
    'telemadrid_schedule_scraper',
    default_args=default_args,
    description='DAG para scrapeo de horarios de TV de Telemadrid',
    schedule_interval='30 00,06,12,18 * * *',
    catchup=False,  # Evitar que se ejecute para fechas pasadas
    tags=["Telemadrid"],
)
def telemadrid_schedule_scraper():
    
    # Función para scrapear la programación de Telemadrid
    @task()
    def scrape_telemadrid_schedule():
        channel_title = 'Telemadrid'
        channel_url = 'https://www.telemadrid.es/programacion/'
    ...
    flow = scrape_telemadrid_schedule() >> merge_all_xml() >> push_all_in_one()

# Para crear el DAG solo tienes que invocar la función `telemadrid_schedule_scraper`
dag_telemadrid = telemadrid_schedule_scraper()
